...
def main(params):
    ...
    for i, ref in enumerate(refs):
        jref = {}
        jref['split'] = ref['split']
        img = refer.loadImgs(ref['image_id'])[0]
        if 'file_name' in img:
            jref['file_path'] = os.path.join(refer.IMAGE_DIR, img['file_name']) # copy it over, might need
        if 'ref_id' in ref:
            jref['id'] = ref['ref_id'] # copy over & mantain an id, if present (e.g. coco ids, useful)

        jref['width'] = img['width']
        jref['height'] = img['height']

        out['images'].append(jref)
