...
def softmax_sample(p):
    ...
    """
    #xp = cuda.get_array_module(p.data)   torch.tensor(np.array(num_labels['T'])).to(device))
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    #p.data.to(device)
    rand = torch.tensor(np.random.uniform(size=p.data.shape[0])).to(device)
    next_state = torch.tensor(np.zeros(p.data.shape[0])).to(device) #, dtype="int32")
    ##To check
        ...
        for i in range(model.seq_length):
            ...
            for prev_net, tokens, sum_likelihood, likelihood in candidates:
                if tokens[-1] == 0:
                    ...
                    continue
                net = prev_net.copy()
                w = torch.tensor(np.asarray([tokens[-1]]).astype(np.int32)).to(device)
                h = net.forward(feat, w, i)
                #
                #token_likelihood = cuda.to_cpu(F.log_softmax(h).data[:,:-1])[0]
                token_likelihood = (F.log_softmax(h).data[:,:-1]).cpu()[0]
...
def make_graph(ve, cca, loader, split, params, xp):
    ...
    with torch.no_grad():
        ...
        while True:
            ...
            feats = torch.tensor(np.array(data['feats'])).to(device)#, dtype=xp.float32))
            ...
            score = (F.matmul(vis_enc_feats, vis_enc_feats, transb=True)).cpu().data
            float_score = []
