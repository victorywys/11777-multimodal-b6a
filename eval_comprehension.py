...
def eval_all(params):
    ...
    with torch.no_grad():
        ...
        lm  = LanguageModel(len(loader.ix_to_word), loader.seq_length).to(device)
        
        #serializers.load_hdf5(osp.join(model_dir, params['id']+params['id2']+"ve.h5"), ve)
        ve = torch.load(osp.join(model_dir, params['id']+params['id2']+"ve.h5")).to(device)
        #serializers.load_hdf5(osp.join(model_dir, params['id']+params['id2']+"le.h5"), le)
        le = torch.load(osp.join(model_dir, params['id']+params['id2']+"le.h5")).to(device)
        #serializers.load_hdf5(osp.join(model_dir, params['id']+params['id2']+"cca.h5"), cca)
        cca = torch.load(osp.join(model_dir, params['id']+params['id2']+"cca.h5")).to(device)
        #serializers.load_hdf5(osp.join(model_dir, params['id']+params['id2']+"lm.h5"), lm)
        lm = torch.load(osp.join(model_dir, params['id']+params['id2']+"lm.h5")).to(device)
        ...
        while True:
            ...
            feats = torch.tensor(data['feats'],dtype=torch.float32).to(device)
            seqz = data['seqz']
            lang_last_ind = calc_max_ind(seqz)
            for i, sent_id in enumerate(sent_ids):
                gd_ix = gd_ixs[i]
                labels = torch.zeros(len(img_ann_ids), dtype=torch.int32).to(device)
                ...
                sent_seqz = torch.tensor(sent_seqz,dtype=torch.int32).to(device)    
