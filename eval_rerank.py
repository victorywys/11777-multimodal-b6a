...
def main(params):
    ...
    with torch.no_grad():
        ...
        ve = VisualEncoder().to_(device)
        if 'attention' in params['id']:
            le = LanguageEncoderAttn(len(loader.ix_to_word)).to(device)
        else:
            le = LanguageEncoder(len(loader.ix_to_word)).to(device)
        cca = CcaEmbedding().to(device)
        #serializers.load_hdf5(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"ve.h5", ve)
        ve = torch.load(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"ve.h5").to(device)
        #serializers.load_hdf5(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"le.h5", le)
        le = torch.load(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"le.h5").to(device)
        #serializers.load_hdf5(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"cca.h5", cca)        
        cca = torch.load(params['model_root']+params['dataset']+'_'+params['splitBy']+'/'+params['id']+"cca.h5").to(device)

        img_to_ref_ids, img_to_ref_confusion = calc_confusion(loader, data, ref_to_beams, ve, le, cca, params)
...
def calc_confusion(loader, data, ref_to_beams, ve, le, cca, params):
    img_to_ref_ids = {}
    for item in data:
        ref_id = item['ref_id']
    ...
    for image_id in tqdm(img_to_ref_ids):
        ...
        for ref_id in img_ref_ids:
            ...
            score = torch.matmul(lang_enc_feats,torch.transpose(vis_enc_feats)).data.detatch().cpu()
